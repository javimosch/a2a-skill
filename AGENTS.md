...
- debri-a2a agent: `bun ~/ai/devin-bridge/src/debri-a2a.ts` (persistent, listens on bus)
...
| Smoke testing with new CLI features uses old installed binary | When testing new CLI commands (e.g., Phase 2 task commands), the installed `a2a` binary does not have the new features. Build from the worktree: `python3 a2a.py > a2a-new && chmod +x a2a-new`, then use `./a2a-new` instead of `a2a`. The installed binary is stale until you run `install.sh`. |
| Agents do not mark themselves done even when work is complete | Agents may complete their assigned work but fail to set `status=done` on the bus. Monitor with `a2a list --json` to see which agents are stuck. If an agent stays `active` after work is done, manually mark it done: `a2a register <agent-id> --status done --upsert`. Consider adding explicit "mark yourself done" instructions to kit prompts. |
| Goal skill for tracking multi-step tasks | The `goal` skill can be used by agents to track progress on complex, multi-step tasks. Invoke with `skill goal` at the start of the task, then use `/goal` commands (add, list, complete) to manage subtasks. This helps agents stay organized and prevents them from forgetting parts of the work. |
| Removing iteration cap for long-running tasks | Default `--max-turns` limits may be too low for complex multi-agent coordination tasks. Use `--max-turns 999999` (or similar high value) in `spawn_team.sh` or `a2a-spawn` to prevent agents from stopping mid-task. Modify the spawn script to add this flag to the CLI invocation. |
| QA agent improves verification quality | Adding a dedicated QA reviewer agent to multi-agent teams catches issues that the implementing agents miss. The QA agent should be spawned separately, review all changes, test the implementation, and report findings back to the team. Pattern: spawn improver-1, improver-2, then qa. |
| Database clearing between smoke tests | When re-running smoke tests on the same project, clear the database first: `rm -rf ~/.a2a/{project-name}`. Otherwise, old agents, messages, and state from previous runs interfere with the new test. This is critical when testing new features like Phase 2 tasks that create schema changes. |
