...
| claude as root: `--allow-dangerously-skip-permissions` blocked | When running as root, claude refuses `--allow-dangerously-skip-permissions` outright. Use `--permission-mode auto --allowedTools "Bash,Read,Edit"` instead, with `CLAUDE_CODE_DANGEROUSLY_SKIP_PERMISSIONS=1`. The `a2a-spawn` script auto-detects root and uses this fallback (fixed in commit 7488f4a). |
